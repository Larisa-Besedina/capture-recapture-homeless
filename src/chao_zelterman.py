...
Обе оценки опираются только на частоты захвата f1 и f2 внутри страты и не
требуют предположения о независимости списков, но при малом f2 неустойчивы.

Используется классическая формула Чао `n + f1²/(2·f2)` (Chao, 1987):
в исходных данных f2 ≥ 4 во всех стратах, поэтому она определена и
воспроизводима читателем по описательной таблице.

Как считаются оценки и интервалы:
  * точечная оценка — по исходным данным (медиана бутстреп-распределения
    нелинейной статистики не является её оценкой);
...
def chao_estimate(n: int, f1: int, f2: int) -> float:
    """
    Оценка Чао (классическая, Chao 1987): n + f1² / (2·f2).
