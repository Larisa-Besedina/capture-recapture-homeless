...
Единая палитра PALETTE — один источник правды для всех рисунков, чтобы цвет
метода не расходился между ними:
...
Размеры шрифтов, маркеров и линий собраны в STYLE: правится в одном месте.
...
STYLE = {
    ...
    "marker": 12,
    "marker_replace": 18,    # треугольники замены (лог-линейные модели) — крупнее
    "marker_obs": 10,
    "line_obs": 2.5,
    "capsize": 5,
    "capthick": 2.0,
...
def plot_method_comparison(all_estimates: pd.DataFrame, strata: str = "total",
    ...
    смещением по X. title=None означает отсутствие заголовка: в статье
    подпись к рисунку идёт отдельно.
    """
